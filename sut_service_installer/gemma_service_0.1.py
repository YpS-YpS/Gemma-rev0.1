...
# Track running background processes for hooks/sideload
...
def execute_command():
    """
    Execute an external command/script on the SUT.

    Supports: .py, .bat, .cmd, .ps1, .exe files
    ...
    Request JSON:
        ...
        working_dir: Working directory (optional, defaults to parent of path)
        wait: If True, wait for completion; if False, return immediately with PID
    ...
    Response (wait=False):
        status: "success"
        pid: Process ID for later termination
    """
    ...
    try:
        ...
        shell = data.get('shell')  # None means auto-detect
        ...
        # Determine if shell mode should be used
        use_shell = shell if shell is not None else ext in ['.bat', '.cmd']
        ...
        if ext == '.py':
            ...
            cmd = [path] + args  # Will run in shell
        elif ext == '.exe':
            cmd = [path] + args
        else:
            # Treat as executable
        ...
        logger.info(f"Executing command: {' '.join(cmd)} (wait={wait}, shell={use_shell})")
        ...
        if wait:
            ...
            try:
                ...
                logger.error(f"Command timed out after {timeout}s: {path}")
                return jsonify({
                    "status": "error",
            ...
            # Asynchronous execution - return PID immediately
            try:
                ...
                logger.info(f"Background process started: {path} (PID: {proc.pid})")
...
def terminate_background_process():
    """
    ...
    Response:
        status: "success" or "error"
        terminated: True if process was terminated
    """
    global background_processes
    ...
    try:
        ...
        # First check our tracked background processes
        with background_processes_lock:
            if pid in background_processes:
                ...
                try:
                    ...
                    proc.wait(timeout=5)
                    terminated = True
                    logger.info(f"Terminated tracked process: PID {pid} ({proc_info['path']})")
                except subprocess.TimeoutExpired:
                    proc.kill()
                    terminated = True
                    logger.warning(f"Force killed tracked process: PID {pid}")
                except Exception as e:
                    logger.warning(f"Error terminating tracked process: {e}")

                del background_processes[pid]

        # If not in our tracked processes, try to terminate by PID directly
        if not terminated:
            try:
                ...
                process.wait(timeout=5)
                terminated = True
                logger.info(f"Terminated process by PID: {pid}")
            except psutil.NoSuchProcess:
                logger.info(f"Process {pid} not found (already terminated?)")
                terminated = True  # Consider it a success if process doesn't exist
            except psutil.TimeoutExpired:
                try:
                    process.kill()
                    terminated = True
